...
class TestAirPollution():
    ...
    def setup_producer(self) -> KafkaProducer:
        topic = SensorType.AIR_POLLUTION.value
    ...
    def setup_sensor(self):
        return AirPollutionSensor(sensor_name = 'test', gather_time = datetime, coordinates = Coordinates(0.0, 0.0), socrates = Random(), temporal_second_delay = 1)
    ...
    def after_test(self, setup_clickhouse):
        query = "DELETE FROM sc_database.air_pollution WHERE sensor_name = 'test';"
    ...
    def test_persistence(self, setup_clickhouse, setup_producer, setup_sensor):
        ...
        while True:
            query = "SELECT sensor_name FROM sc_database.air_pollution WHERE sensor_name = 'test';"
