...
class TestChargingStation():
    ...
    def setup_producer(self) -> KafkaProducer:
        topic = SensorType.CHARGING_STATION.value
    ...
    def setup_sensor(self):
        return ChargingStationSensor(sensor_name = 'test', gather_time = datetime, coordinates = Coordinates(0.0, 0.0), socrates = Random(), temporal_second_delay = 1)
    ...
    def after_test(self, setup_clickhouse):
        query = "DELETE FROM sc_database.charging_station WHERE sensor_name = 'test';"
    ...
    def test_persistence(self, setup_clickhouse, setup_producer, setup_sensor):
        ...
        while True:
            query = "SELECT sensor_name FROM sc_database.charging_station WHERE sensor_name = 'test';"
