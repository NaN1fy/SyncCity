...
class TestPaymentStation():
    @pytest.fixture(scope = 'class')
    def setup_producer(self) -> KafkaProducer:
        topic = SensorType.PAYMENT_STATION.value
    ...
    def setup_sensor(self):
        return PaymentStationSensor(sensor_name = 'test', gather_time = datetime, coordinates = Coordinates(0.0, 0.0), socrates = Random(), temporal_second_delay = 1)
    ...
    def after_test(self, setup_clickhouse):
        query = "DELETE FROM sc_database.payment_station WHERE sensor_name = 'test';"
    ...
    def test_persistence(self, setup_clickhouse, setup_producer, setup_sensor):
        ...
        while True:
            query = "SELECT sensor_name FROM sc_database.payment_station WHERE sensor_name = 'test';"
