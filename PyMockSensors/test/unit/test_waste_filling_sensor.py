    ...
    with patch.object(Random, 'uniform', return_value = 0):
        with patch('uuid.uuid4') as mocked_id:
            ...
            with patch('datetime.datetime') as mocked_datetime:
                ...
                sensor_simulator = WasteFillingSensor("Arcella", mocked_datetime, coordinates, random_obj, 1)
                sensor_simulator._WasteFillingSensor__sensor_id = simulated_id
                json_data = sensor_simulator.simulate()
                parsed_json = json.loads(json_data)
                expected_json = {
                    "sensor_type": "waste_filling",
                    "sensor_name": "Arcella",
                    "sensor_id": "mocked-id",
