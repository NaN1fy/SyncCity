...
def test_charging_station_sensor():
        ...
        with patch.object(Random, 'random', return_value = 0):
            with patch('uuid.uuid4') as mocked_id:
                ...
                with patch('datetime.datetime') as mocked_datetime:
                    with patch.object(ChargingStationSensor, '_ChargingStationSensor__generate_plate', return_value="AA000AA"):
                        mocked_datetime.now.return_value = simulated_datetime
                        mocked_id.return_value = simulated_id
                        sensor_simulator = ChargingStationSensor("Arcella", mocked_datetime, coordinates, random_obj, 1)
                        sensor_simulator._ChargingStationSensor__sensor_id = simulated_id
                        json_data = sensor_simulator.simulate()
                        parsed_json = json.loads(json_data)
                        print(parsed_json)
                        expected_json = {
                            "sensor_type": "charging_station",
                            "sensor_name": "Arcella",
                            "sensor_id": "mocked-id",
