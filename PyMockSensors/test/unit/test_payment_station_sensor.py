...
def test_payment_station_sensor():
            ...
            with patch('uuid.uuid4') as mocked_id:
                ...
                with patch.object(Random, "uniform", return_value = 1):
                    with patch('datetime.datetime') as mocked_datetime:
                        ...
                        sensor_simulator = PaymentStationSensor("Arcella", mocked_datetime, coordinates, random_obj, 0)
                        sensor_simulator._PaymentStationSensor__sensor_id = simulated_id
                        json_data = sensor_simulator.simulate()
                        parsed_json = json.loads(json_data)
                        expected_json = {
                            "sensor_type": "payment_station",
                            "sensor_name": "Arcella",
                            "sensor_id": "mocked-id",
                            "gather_time": "2024-04-26 12:10:00.123456",
                            "readings": [{"type": "€/kWh", "value": 0.0}],
