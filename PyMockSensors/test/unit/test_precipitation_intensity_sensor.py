...
def test_precipitation_intensity_sensor():
            ...
            with patch.object(Random, 'random', return_value = 0):
                with patch('uuid.uuid4') as mocked_id:
                    ...
                    with patch('datetime.datetime') as mocked_datetime:
                        ...
                        sensor_simulator = PrecipitationIntensitySensor("Arcella", mocked_datetime, coordinates, random_obj, 1)
                        sensor_simulator._PrecipitationIntensitySensor__sensor_id = simulated_id
                        json_data = sensor_simulator.simulate()
                        parsed_json = json.loads(json_data)
                        print(parsed_json)
                        expected_json = {
                            "sensor_type": "precipitation_intensity",
                            "sensor_name": "Arcella",
                            "sensor_id": "mocked-id",
                            "gather_time": "2024-04-26 12:10:00.123456",
                            "readings": [{"type": "mm/h", "value": 0.0}],
